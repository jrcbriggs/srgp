...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ru = TableWardUpdate(table=self.table, csv_basename=self.csv_basename, **self.config)
    # ConfigHandler
    ...
    def test_append_fields_other(self):
        for isness in ('is_supporter', 'is_volunteer'):
            ...
            self.ru.extra_fields(row0, fields_extra)
    ...
    def test_clean_value(self):
        ...
        for fixture, expected in fixtures.items():
            actual = self.ru.clean_value(fixture)
    ...
    def test_fix_addresses(self):
        ...
        self.ru.fix_addresses_city_postcode_countrycode(row, self.address_fields)
    ...
    def test_fix_address_street_1(self):
        ...
        exp = {'junk': 1, 'A1': '220 SV Road','A2': 'Crookes', 'A3': '',
               'A4': '', 'A5': 'Sheffield', 'A6': 'S10 1ST', 'A7': 'GB', }
        self.ru.fix_address_street(row, self.address_fields)
        print('row',row)
        self.assertDictEqual(row, exp)
    ...
    def test_fix_append_fields(self):
        ...
        self.ru.extra_fields(self.row0, fields_new)
    ...
    def test_fix_append_fields_support_level_member(self):
        ...
        self.ru.extra_fields(self.row0, {'support_level': 'support_level'})
        self.assertDictEqual(self.row0, expected)
    ...
    def test_fix_doa(self):
        ...
        self.ru.fix_doa(row, self.doa_fields)
        self.assertDictEqual(row, expected)

    def test_fix_local_party(self):
        self.row0.update({'Local party': None})
        self.ru.fix_local_party(self.row0)
        expected = self.row0.copy()
    ...
    def test_fix_postcode(self):
        ...
        self.ru.fix_postcode(row, self.address_fields, 'A6')
        self.assertDictEqual(row, exp)
    ...
    def test_flip_fields(self):
        ...
        self.ru.flip_fields(row, fieldnames)
    ...
    def test_get_status(self):
        for k, v in {'Cancelled': 'canceled', 'Current': 'active', 'Deceased': 'expired', 'Expired': 'expired', 'New': 'active'}.items():
            ...
            self.ru.fix_status(self.row0)
            actual = self.row0['Status']
            expected = v
            self.assertEqual(actual, expected, 'k {} v {}'.format(k, v))

    def test_iscity(self):
        self.assertTrue(self.ru.iscity('Sheffield'), 'expected match:' + 'Sheffield')
        self.assertTrue(self.ru.iscity('sheffield'), 'expected match:' + 'Sheffield')
        self.assertFalse(self.ru.iscity('XSheffield'), 'unexpected match:' + 'ShefXield')
        self.assertFalse(self.ru.iscity('ShefXield'), 'unexpected match:' + 'ShefXield')
        self.assertFalse(self.ru.iscity('SheffieldX'), 'unexpected match:' + 'SheffieldX')

    def test_iscounty(self):
        self.assertTrue(self.ru.iscounty('South Yorks'), 'expected match:' + 'South Yorks')

    def test_isdeceased_true(self):
        self.row0.update({'Status': 'Deceased'})
        actual = self.ru.isdeceased(self.row0)
    ...
    def test_isdeceased_no_field(self):
        actual = self.ru.isdeceased(self.row0)
        self.assertFalse(actual)
        self.assertFalse(self.ru.iscounty('XSouth Yorks'), 'unexpected match:' + 'XSouth Yorks')

    def test_ishouse(self):
        for house in ('Barn', 'Building', 'College',):
            self.assertTrue(self.ru.ishouse('123' + house), 'expected match:' + house)
        for house in('Sheffield', 'South Yorks', 'Approach', 'S10 1ST',):
            self.assertFalse(self.ru.ishouse(house), 'unexpected match:' + house)
    ...
    def test_ismember_true(self):
        ...
        for status in ('Current', 'New'):
            self.row0.update({'Status': status})
            self.ru.fix_status(self.row0)
            actual = self.ru.is_party_member(self.row0)
    ...
    def test_ispostcode(self):
        for postcode in('S10 1ST', 'S1 1ST', 'S1 2ST',):
            self.assertTrue(self.ru.ispostcode(postcode), 'expected match:' + postcode)
        for postcode in('SX10 1ST', 'S1 12ST', 'S1 1STX', 'S1 X1ST',):
            self.assertFalse(self.ru.ispostcode(postcode), 'unexpected match:' + postcode)

    def test_isstreet(self):
        for street in 'Approach Drive Place'.split():
            self.assertTrue(self.ru.isstreet('123' + street), 'expected match:' + street)
#         self.assertTrue(self.ru.isstreet('Kelham Island'), 'expected match:' + street)
        self.assertTrue(self.ru.isstreet('220 Stannington View Road'), 'expected match:' + street)
        for street in 'Sheffield S10 Yorks'.split():
            self.assertFalse(self.ru.isstreet(street), 'unexpected match:' + street)

    def test_isvoter_register_true(self):
        for status in ('E'):
            self.row0.update({'Status': status})
            actual = self.ru.isvoter(self.row0)
            self.assertTrue(actual)
    ...
    def test_is_matching_row(self):
        skip_dict = {'a': '0'}
        actual = self.ru.is_matching_row(self.row0, skip_dict)
    ...
    def test_tags_create(self):
        ...
        actual = self.ru.tags_create(row, self.tagfields, csv_basename)
        self.assertEqual(expected, actual)
